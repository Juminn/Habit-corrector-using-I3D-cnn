...
flags.DEFINE_integer('batch_size', 3, 'Batch size.') # 내가 6->4 줄임
flags.DEFINE_integer('num_frame_per_clib', 16, 'Nummber of frames per clib') # 내가64-> 32
...
flags.DEFINE_integer('classics', 4, 'The num of class') # 101 -> 1 내가줄임
FLAGS = flags.FLAGS
model_save_dir = './models/rgb_scratch_10000_6_64_0.0001_decay'

os.environ["CUDA_VISIBLE_DEVICES"] = "0" #내가
...
def run_training():
    ...
    rgb_pre_model_save_dir = "C:\\project\\I3D_Tensorflow_master\\checkpoints\\rgb_imagenet"
    ...
    with tf.Graph().as_default():
        ...
        config = tf.ConfigProto(allow_soft_placement=True) #내가
        #config.gpu_options.per_process_gpu_memory_fraction = 0.4 #내가
        sess = tf.Session(
                        config=config #내가
                        )
        sess.run(init)
    ...
    #ckpt = tf.train.get_checkpoint_state(rgb_pre_model_save_dir) 내가 #넣음
    ckpt = 1 # 내가
    model_checkpoint_path = '..\\..\\checkpoints\\rgb_imagenet\\model.ckpt'#내가넣음
    if ckpt and model_checkpoint_path:
        print("loading checkpoint %s,waiting......" % model_checkpoint_path) #
        rgb_saver.restore(sess, model_checkpoint_path)
        print("load complete!")
