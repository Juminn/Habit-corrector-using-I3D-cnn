...
def get_data(filename, num_frames_per_clip, sample_rate, s_index=-1):
    ...
    #for parent, dirnames, filenames in os.walk(filename):
        #print( "dir, file : ",dirnames, filenames)#내가
    if len(filenames)==0:
    ...
    if (len(filenames)-s_index) <= num_frames_per_clip:
        ...
        if len(filenames) < num_frames_per_clip:
            for i in range(num_frames_per_clip):
                if i >= len(filenames):
                    i = len(filenames)-1
                image_name = os.path.join(str(filename) , str(filenames[i])) #내가
                img = Image.open(image_name)
                img_data = np.array(img)
                ret_arr.append(img_data)
        else:
            for i in range(num_frames_per_clip):
                image_name = os.path.join(str(filename) , str(filenames[len(filenames)-num_frames_per_clip+i])) #내가
                img = Image.open(image_name)
                img_data = np.array(img)
                ret_arr.append(img_data)
    ...
    filenames = sorted(filenames)
    #print("sorted filenames : ", filenames)  # 내가
    if s_index < 0:

        s_index = random.randint(0, len(filenames) - num_frames_per_clip)
    for i in range(int(num_frames_per_clip/sample_rate)):
        image_name = os.path.join(str(filename), str(filenames[int(i*sample_rate)+s_index]))
        #print("image name:",image_name)#내가
        img = Image.open(image_name)
        img_data = np.array(img)
        ret_arr.append(img_data)
    print("image name:",image_name)#내가
...
def get_frames_data(filename, num_frames_per_clip, sample_rate, add_flow):
    ...
    #print(filename)
    rgb_ret_arr, s_index = get_data(filename, num_frames_per_clip, sample_rate)  # 내가 filename-> filename_i
...
def data_process(tmp_data, crop_size):
    ...
    for j in xrange(len(tmp_data)):
        ...
        if img.width > img.height:
            scale = float(256) / float(img.height)
            img = np.array(cv2.resize(np.array(img), (int(img.width * scale + 1), 256))).astype(np.float32)
            #print("if img.shape: ", img.shape)  # 내가
        else:
            scale = float(256) / float(img.width)
            img = np.array(cv2.resize(np.array(img), (256, int(img.height * scale + 1)))).astype(np.float32)
            #print("else img.shape: ", img.shape)  # 내가
        if j == 0:
            # crop_x = random.randint(0, int(img.shape[0] - crop_size))
            # crop_y = random.randint(0, int(img.shape[1] - crop_size))
            crop_x = int((img.shape[0] - crop_size) / 2)
            crop_y = int((img.shape[1] - crop_size) / 2)
            #print("ifj img.shape: ", img.shape)  # 내가
       # print("before img.shape: ", img.shape)  # 내가
        img = img[crop_x:crop_x + crop_size, crop_y:crop_y + crop_size, :]
        #print("after img.shape: ", img.shape )#내가
        img_datas.append(img)
    return img_datas

def read_clip_and_label(filename, batch_size, start_pos=-1, num_frames_per_clip=64, sample_rate=1, crop_size=224, shuffle=True, add_flow=False): #내가 same 4->1 로
    ...
    if start_pos < 0:
        shuffle = True
    if shuffle:
        video_indices = list(range(len(lines))) #내가
    ...
    for index in video_indices:
        ...
        if len(tmp_rgb_data) != 0:
            ...
            if add_flow:
                ...
                #print("flow_img_datas: ", flow_img_datas)#내가
                flow_data.append(flow_img_datas)
            rgb_data.append(rgb_img_datas)
            #print("rgb", len(rgb_data), len(rgb_img_datas))#내가
    ...
    #print("rgb", rgb_data[0])#내가
    #print("rgb", len(rgb_data), len(rgb_data[0])) #내가
    #for i in range(len(rgb_data)): #내가
    #    for j in range(len(rgb_data[i])):
    #        print(rgb_data[i][j].shape)
    #print("rgb", rgb_data[0][0])  # 내가
    #print("rgb", rgb_data[0][0].shape)  # 내가
